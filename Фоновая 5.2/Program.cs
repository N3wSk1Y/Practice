        ...
        while (keyInput.Key != ConsoleKey.F9)
        {
            switch (keyInput.Key)
            {
                case ConsoleKey.D1:
                    ...
                    break;
                case ConsoleKey.D2:
                    Console.WriteLine($"Dень недели, с которого начался месяц - {(Week)weather.Day}"); keyInput = Console.ReadKey();
                    break;
                case ConsoleKey.D3:
                    ...
                    break;
                case ConsoleKey.D4:
                    Console.WriteLine($"Kоличество дней с температурой в ноль градусов - ${weather.GoodDays}"); keyInput = Console.ReadKey();
                    break;
                case ConsoleKey.D5:
                    Console.WriteLine($"Mаксимальный скачок температуры за месяц - {weather.HighestRise()}"); keyInput = Console.ReadKey();
                    break;
                case ConsoleKey.D6:
                    Console.WriteLine($"Mаксимальный скачок температуры за месяц - {weather.HighestRise(out temp)}, он был {temp}-го"); keyInput = Console.ReadKey();
                    break;
            }
        }            
    }
